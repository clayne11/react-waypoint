...
describe('<Waypoint>', function() {
  ...
  beforeEach(() => {
    ...
    props = {
      onEnter: jest.fn().mockName('onEnter'),
      onLeave: jest.fn().mockName('onLeave'),
      onPositionChange: jest.fn().mockName('onPositionChange'),
    };
    ...
    parentStyle = {
      ...
      margin: margin, //Normalize the space above the viewport.
    };
  ...
  describe('when scrolling while the waypoint is visible', () => {
    ...
    it('does not call the onEnter handler again', () => {
      expect(props.onEnter.mock.calls.length).toBe(1);
    });
    ...
    it('does not call the onPositionChange handler again', () => {
      expect(props.onPositionChange.mock.calls.length).toBe(1);
    });
  });
  ...
  describe('when the Waypoint is below the bottom', () => {
    ...
    describe('with children', () => {
      ...
      it('calls the onEnter handler when scrolling down far enough', () => {
        component = subject();
        props.onPositionChange.mockClear();
  ...
  describe('when the Waypoint has children', () => {
    ...
    it('errors when multiple children are provided', () => {
      ...
      props.children = [<div key={1} />, <div key={2} />, <div key={3} />];

      // eslint-disable-next-line no-console
      console.error(
        'NOTE: Expect an error log below. You can ignore it. ' +
          "We're testing errors thrown and it seems hard to prevent things " +
          'from being logged.'
      );
      expect(subject).toThrowError(notValidErrorMessage);
    ...
    it('does not throw with a Stateless Component as a child', () => {
      const StatelessComponent = curProps => <div ref={curProps.innerRef} />;
  ...
  describe('when the Waypoint has children and is above the top', () => {
    ...
    beforeEach(() => {
      ...
      props.onEnter.mockClear();
      props.onLeave.mockClear();
      scrollNodeTo(scrollable, 400);
    });
  ...
  it('does not throw an error when window is undefined', () => {
    jest.spyOn(Waypoint, 'getWindow').mockImplementation(() => undefined);
    expect(subject).not.toThrow();
  });
...
describe('<Waypoint> Horizontal', function() {
  ...
  beforeEach(() => {
    ...
    parentStyle = {
      ...
      width: parentWidth,
      // eslint-disable-next-line object-shorthand
      margin: margin, //Normalize the space left of the viewport.
    };
